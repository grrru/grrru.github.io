...
async def main():
    ...
    task1 = asyncio.create_task(say(1, "hello3"))
    task2 = asyncio.create_task(say(2, "hello4")) 
...
1. coroutine 객체를 만들고 await, 만들고 await
    - 만들어진 `hello1`가 이벤트 루프에 스케줄링되고 실행된다.
    - `hello1`이 sleep을 한 후 대기 상태로 바뀌면, 이벤트 루프가 다음 실행할 coroutine을 확인한다.
    - 하지만 등록된 다른 coroutine이 없기 때문에 무작정 대기한다.
    - `hello1` 동작이 끝난 후 `hello2`가 이벤트 루프에 등록되어 실행되어 약 2초 소요.

1. `create_task`로 두 coroutine 객체를 이벤트 루프에 등록

    - main 스레드가 `task1`, `task2`를 생성한 후 `await task1`으로 대기한다. 이 때 이벤트 루프가 제어권을 가져간다.
    - `hello3`이 sleep 후 이벤트 루프가 다음에 실행할 coroutine(`hello4`)을 찾아서 실행한다.
...
`asyncio.Queue`는 coroutine 간에 데이터를 안전하게 전달하는 큐다.
...
async def main():
    queue = asyncio.Queue()
    await asyncio.gather(producer(queue=queue), consumer(queue=queue))
...
- `put(item)`: 큐에 데이터를 `maxsize`까지 넣을 수 있다. 큐가 가득 차면 대기한다.
  - `maxsize` 기본 값은 0(무제한)
- `get()`: 큐에서 데이터를 꺼낸다. 큐가 비어 있으면 대기한다.
- `task_done()`: `get()`으로 꺼낸 아이템의 처리가 끝났음을 알린다.
  - `task_done`을 호출하지 않으면 `join`이 모든 처리가 끝났음을 인지하지 못하고 무한 대기한다.
- `join()`: 큐에 넣은 모든 아이템이 `task_done()`될 때까지 대기한다.

### 사용 이유

- 생산 속도와 소비 속도가 다를 때, 큐가 버퍼 역할을 할 수 있다.
- 디커플링: 생산자는 소비자의 처리 상태를 알지 않아도 된다.
- back pressure: `maxsize`로 크기를 제한하면 생산자가 알아서 대기한다.

## asyncio 동기화

`asyncio`를 사용할 때, 여러 coroutine이 공유 리소스에 접근하는 등의 경우, 실행 흐름을 조율해야 한다.

### asyncio.Semaphore - 동시 실행 수 제한

외부 API에 한 번에 많은 요청을 보내는 경우 rate limit에 걸릴 수 있다. 이런 경우 Semaphore로 동시 실행 수를 제한할 수 있다.
...
async def main():
    ...
    urls = [f"example.com/{i}" for i in range(10)]
...
Go의 channel과 비슷한 것 같다. 차이점은 Go에서 데이터를 공유하는 통로로 설계된 channel을 Semaphore 패턴처럼 사용하는 것이고, Python의 Semaphore는 말 그대로 Semaphore를 위한 것이라는 점이다.

### asyncio.Lock

공유 리소스에 여러 Coroutine이 접근해야 할 때 Lock을 사용할 수 있다.
...
`async with lock`을 대기하는 Coroutine은 대기 상태로 들어가고 이벤트 루프에 제어권을 넘겨준다.  
`await` 없이 동기적으로 실행되는 코드 블록에는 Lock이 필요 없지만 `await`으로 제어권을 넘겨주는 경우 다른 Coroutine이 같은 데이터를 수정할 수 있으므로 Lock이 필요할 수 있다.

### asyncio.Event - Coroutine 간 신호 전달

어떤 Coroutine이 준비가 됐음을 다른 Coroutine에게 알려야 할 때 사용한다.
...
async def waiter(event: asyncio.Event, name: str):
    print(f"{name}: event waiting...")
    await event.wait()
    print(f"{name}: event set!")
...
async def main():
    ...
    await asyncio.gather(
        waiter(event, "Worker A"), waiter(event, "Worker B"), setter(event=event)
    )
...
`wait()`으로 `event.set()`이 호출될 때까지 대기한다.

### asyncio.wait

`gather`는 모든 작업이 끝날 때까지 기다리지만, `wait`은 더 유연한 조건을 지원한다.
