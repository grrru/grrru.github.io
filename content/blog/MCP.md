...
**구간 ①  Host ↔ LLM**
이 구간은 MCP가 아니라 각 LLM 벤더의 API 규격을 따른다. Host는 사용자 메시지와 사용 가능한 도구 정의를 벤더 고유 형식으로 LLM에게 전달한다.

LLM은 전달받은 도구의 name, description, inputSchema와 대화 맥락을 바탕으로 호출할 도구와 인자를 선택한다. 그 결과는 OpenAI, Anthropic, Google 등 벤더별 tool-call 형식으로 Host에 돌아오며, JSON-RPC가 아니다.

즉 LLM이 MCP의 JSON-RPC를 직접 만들어내는 것이 아니다. MCP는 이 Host ↔ LLM 구간에는 직접 관여하지 않는다.

**구간 ②  MCP Client ↔ MCP Server**
Host는 LLM의 벤더 고유 tool-call 응답을 파싱한다.
그 후 Host 내부의 MCP Client가 이를 MCP 표준 메시지, 즉 JSON-RPC 2.0 기반 요청으로 변환하여 MCP Server에 보낸다.

예를 들어 LLM이 "create_github_issue 도구를 repo=myorg/myrepo, title=버그 수정 필요로 호출하라"고 응답하면,
Host/MCP Client는 이를 MCP의 tools/call JSON-RPC 요청으로 변환한다.
